...
describe('About Functions', function() {
  ...
  it('should return a string with a defined suffix', () => {
    function addSuffix(task, suffix) {
      return `${task} ${suffix}`
    }

    function addSymbol(task, symbol) {
      return `${task}${symbol}`
    }

    function markTaskDone(task, suffix) {
      return addSuffix(task, 'done!')
    }

    function hemphasis(task, suffix) {
      return addSymbol(task, '!')
    }
